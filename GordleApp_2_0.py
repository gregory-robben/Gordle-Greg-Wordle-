...
def allowed_word_list(difficulty):
  ...
  try: 
    ...
    if difficulty == 5: #only if the difficulty matches traditional Wordle do we combine the traditional Wordle list with the OED.
      with open('allowed_words.txt',encoding="utf8") as file:
        for line in file:
    ...
    print("Word list imports have failed please check that they are in the game's folder")
    exit()

def set_target(difficulty = 5):
    '''
    set_target chooses a word of specified dificulty length from Official_Worlde_List.txt at random
    Parameter: difficulty (default is 5)
    Returns: targetWord
    '''
    GameDictionary = allowed_word_list(difficulty)
    targetWord = GameDictionary[random.randint(0,len(GameDictionary)-1)]
    return targetWord
...
def user_guess(difficulty = 5):
    ...
    while True:
        ...
        if len(guess) != difficulty:
            ...
            time.sleep(1)
            continue
        if not guess in allowed_word_list(difficulty):
            print("\033[1A\033[K\033[1A")
...
def build_wordle_list():
  wordle_list = []
  with open('Official_Wordle_List.txt',encoding="utf8") as file:
    for line in file:
...
def todays_wordle():
  while True:
    ...
    if play_today == "End":
      ...
      chosenDifficulty = 5
      difficulty =  "Beginner"
      wordle_list = build_wordle_list()
      target = wordle_list[todays_wordle] 
